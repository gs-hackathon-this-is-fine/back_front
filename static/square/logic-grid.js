    ...
    start() {
        ...
        if (this.img === 2) {
            fetch("http://localhost:8000/register/", {
                ...
                body: JSON.stringify({
                    le_cookie: pseudoCookie,
                    fname: document.getElementById("fname").value,
                    lname: document.getElementById("lname").value,
                    sex: document.getElementById("sex").value,
                    ethnicity: document.getElementById("ethnicity").value,
                    uni: document.getElementById("uni").value,
                    grad_date: document.getElementById("grad-date").value
            ...
            fetch("http://localhost:8000/submit/", {
                method: 'POST', // *GET, POST, PUT, DELETE, etc.
                body: JSON.stringify({
                    le_cookie: pseudoCookie, ans: document.querySelector('input[name="ans"]:checked').value
                }) // body data type must match "Content-Type" header
            });
...
var map = {
    ...
    quizes: {
        "25,21": new Quiz(registration, 2),
        "31,32": new Quiz(quizes[0], 1),
        "12,34": new Quiz(quizes[1], 1),
        "27,12": new Quiz(quizes[2], 1),
        "15,11": new Quiz(quizes[3], 1)
    },
...
Game.load = function () {
    return [
        Loader.loadImage('tiles', 'static/assets/Modern_Office_Revamped/3_Modern_Office_Shadowless/Modern_Office_Shadowless_48x48.png'),
        Loader.loadImage('hero', 'static/assets/intern1.png'),
        Loader.loadImage('gryka', 'static/assets/gryka.png'),
        Loader.loadImage('wiki', 'static/assets/wiki.png'),
    ];
};
